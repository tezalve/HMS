...
// *START Select2 Portion*******************************************************************************  
  ...
  $('input[type=checkbox][name=add_more_product]').change(function() {



      if($('input[name=add_more_product]:checked').val() == 1){

        console.log($('input[name=add_more_product]:checked').val());


        var $serial_number_dg = $('.serial_number_dg').select2({
          ...
          $serial_number_dg.on("select2:select", function(e) {
              $("#balance").val($(this).select2('data')['0']['current_stock']);
              $("#quantity").focus();
          });

          $serial_number_dg.on("select2:unselect", function(e) {
              $("#balance").val('');
          });
          ...
          var $item_info_name = $('.item_info_name').select2({
              ...
              ajax: {
                  dataType: 'json',
                  url: "/getItemInfoByQuotedata",
                  delay: 100,
                  data: function(params) {
                      return {
                          term: params.term,
                          main_tran_master_id : $("#main_tran_master_id").val(),
                      }
                  },
                  processResults: function(data, params) {
                      params.page = params.page || 1;
          ...
          $item_info_name.on("select2:select", function(e) {
              $("#rate").val($(this).select2('data')['0']['rate']);
              $("#qty").val($(this).select2('data')['0']['quantity']);
              $("#line_total").val($(this).select2('data')['0']['line_total']);
              $('#serial_number').val(null).trigger("change");
...
//Start  sales-invoice-table DataTable  **********************************************************************
        ...
        //When add button clicked
          ...
          event.preventDefault();
              ...
              if(rate<0){
                alert("You can't add Invalid rate");
                  return;
              }
              ...
              var entry = [
                  ...
                  '<input  type="text"    name="serial_number[]"   class="form-control serial_number_dg"  style="width: 100%;text-align:left;"  value="' + serial_number + '"    readonly>',
        ...
        // End (Quantity*Rate) = Line Cost Calculation
          ...
          // $('#sales-invoice-table tbody').keyup(function(e) {
          //   totalcalculat();
          // }); 
       ...
       payment_method_table = $("#payment-method-table").DataTable({
        ...
        //When addPayment button clicked
          ...
          event.preventDefault();
              ...
              // var data                  = parseFloat(due_amount).toFixed(0);
              // var payment_data          = parseFloat(payment_method_amount).toFixed(0);
            ...
            if(parseFloat(due_amount)<parseFloat(payment_method_amount)){
              ...
              '<input  type="number"    name="payment_method_amount[]"  oninput="calculate();" id="payment_method_amount" class="form-control payment_method_amount"  style="width: 100%;text-align:left;"  value="' + payment_method_amount + '"   >',
              '<input  type="text"    name="payment_method_note[]"  oninput="calculate();" id="payment_method_note" class="form-control payment_method_note"  style="width: 100%;text-align:left;"  value="' + payment_method_note + '"   >',
              '<button class="btn btn-danger btn-block delete-button btn-flat" id="' + '" style="padding: 3px 10px;">Delete</button>',
...
//Start Due Amount Calculation ******
        ...
        due_amount_calculation = function(){
                    ...
                    if(isNaN(parseFloat($('#payment').val()))){
                     ...
                     if(isNaN(parseFloat($('#payment_method_amount').val()))){
                      payment_method_amount = 0;
                    }else{
                      payment_method_amount = parseFloat($('#payment_method_amount').val());
                    }
        ...
        $("#btnSubmit").val('Please wait..');
           ...
           $.ajax({
            ...
            if(data.success == true) {
                  ...
                  window.location.replace('/quote_to_invoice');
