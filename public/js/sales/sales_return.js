...
// *START Select2 Portion******************************************************************************* 
      ...
      $customer=$('.customer').select2({
         ...
         $customer.on("select2:select", function(e) {
          ...
          $invoice_data =$('.sales_invoice_id').select2({
              placeholder: 'Select Sales Invoice',
              allowClear: true,

              ajax: {
                  dataType: 'json',
                  url: "/getSalesInvoiceListData",
                  delay: 250,
                  data: function(params) {
                      return {
                          term: params.term,
          ...
          $invoice_data.on("select2:select", function(e) {
             ...
             $("#sales_invoice_master_id").val($(this).select2('data')['0']['sales_invoice_master_id']);
           dataload(); 
          ...
          dataload = function(){
            ...
            $.ajax({
              type:   'GET', 
              url :   "/getSalesInvoiceDataByInvoiceNo",
              headers:{
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                      },        
              data:   {
                         master_id: $("#sales_invoice_id").val(),
                      },          
              dataType: 'json',
              success: function(data) {
                var dataSet = data.data;
                  table = $('#sales-invoice-table').DataTable( {
                    destroy:    true,
                    paging:     false,
                    searching:  false,
                    ordering:   false,
...
//Start Due Amount Calculation ******
        ...
        due_amount_calculation = function(){
                    ...
                    $('#return_amount').val(line_total-discount_amount-restock_amount-payment);        
...
/*********Warranty Expired Calculation**********/
  ...
  /*********Warranty Expired Calculation**********/
    ...
    $('#sales_return_form').on('submit',(function(e) {
           e.preventDefault();
        ...
        $("#btnSubmit").val('Please wait..');
           ...
           $.ajax({
            ...
            if(data.success == true) {
                  ...
                  window.location.replace('/sales_return');
